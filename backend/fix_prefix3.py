...
# Find the dynamic world closing: after d-boss reward, before ]),
dynamic_close = '                 _reward(diamonds=40,coins=20,badges=["dp-master"],unlocks_town="town-2-alpine")),\n        ]),\n    ]\n)\n\n# ═══════════════════════════════════════════════════════════════\n# WORLD 12: Alpine Summit (advanced)'
...
if dynamic_close in content:
    content = content.replace(dynamic_close, prefix_town + dynamic_close)
    print('PREFIX TOWN INSERTED INTO DYNAMIC WORLD')
else:
    print('DYNAMIC CLOSE MARKER NOT FOUND')
    # Try to find the dynamic world end
    idx = content.find('WORLD_DYNAMIC')
    if idx != -1:
        print('WORLD_DYNAMIC found at', idx)
        # Show next 500 chars
