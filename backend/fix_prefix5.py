...
# Find the end of dynamic world by looking for the pattern after d-boss
# The dynamic world ends with: _reward(...), then ]), then ] then )
# Then a blank line, then the Alpine Summit comment

# Search for the unique string that marks the end of dynamic world
marker = 'town-2-alpine")),'
...
# Find the next 3 lines: ]), then ] then )
rest = content[line_end:line_end+20]
print('REST:', repr(rest))

# The structure should be:
# \n        ]),\n    ]\n)\n\n# ════════...
# We want to insert before the first ]),
...
# Insert after the d-boss reward line's closing
insertion_point = content.find(marker) + len(marker)
# Find the next \n after insertion_point
next_newline = content.find('\n', insertion_point)
# The pattern should be: _reward(...)),\n        ]),\n    ]\n)\n\n# ...
# We want to insert after the first ]), which closes the memo town
# Actually we want to insert after the entire memo town closes: ]),
# Let's find the first ]), after the marker

after_marker = content[insertion_point:]
# Find the first occurrence of ]),
close_idx = after_marker.find('        ]),')
if close_idx == -1:
    print('CLOSE IDX NOT FOUND')
    exit(1)

# Insert prefix_town after this close
insert_pos = insertion_point + close_idx + len('        ]),')
content = content[:insert_pos] + '\n' + prefix_town + content[insert_pos:]
