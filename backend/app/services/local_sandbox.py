...
_BATCH_RUNNER = (
    ...
    "    sys.exit(0)\n"
    # Auto-detect a single top-level, return-style function so function-based
    # submissions (two_sum, missing_number, ...) grade correctly even when no
    # function_name was supplied by the caller. Kept conservative: program-style
    # submissions reading stdin, or code with helpers/zero/multiple defs, stay
    # in program mode exactly as before.
    "def _auto_func(_src):\n"
    "    try:\n"
    "        _tree = ast.parse(_src)\n"
    "    except BaseException:\n"
    "        return ''\n"
    "    _top_defs = [n for n in _tree.body if isinstance(n, ast.FunctionDef)]\n"
    "    if len(_top_defs) != 1:\n"
    "        return ''\n"
    "    _cand = _top_defs[0]\n"
    "    if not any(isinstance(n, ast.Return) for n in ast.walk(_cand)):\n"
    "        return ''\n"
    "    if 'input(' in _src:\n"
    "        return ''\n"
    "    return _cand.name\n"
    "_FUNC_NAME = _FUNC_NAME or _auto_func(_USER_CODE)\n"
    "_FN = None\n"
    "if _FUNC_NAME:\n"
    "    try:\n"
    ...
    "for _raw in _CASES:\n"
    "    try:\n"
    "        # Structured test-case inputs arrive as lists/dicts, not strings.\n"
    "        # Canonicalize once so _raw.strip()/StringIO always see a string.\n"
    "        if not isinstance(_raw, str):\n"
    "            _raw = json.dumps(_raw)\n"
    "        if _FN is not None:\n"
    "            _args = ast.literal_eval('(' + _raw + ')') if _raw.strip() else ()\n"
    "            # Test inputs are arg-lists: a parsed list's ELEMENTS are the\n"
    "            # positional args (parens don't tuplify a single list literal,\n"
    "            # so wrap-only-if-not-tuple would pass the whole list as ONE\n"
    "            # arg). Scalars/strings still wrap as a single argument.\n"
    "            if isinstance(_args, list):\n"
    "                _args = tuple(_args)\n"
    "            elif not isinstance(_args, tuple):\n"
    "                _args = (_args,)\n"
