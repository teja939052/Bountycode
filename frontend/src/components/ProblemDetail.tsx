...
import {
  ...
  BarChart3, ArrowLeft, Sparkles, Lightbulb,
  TrendingUp, Users, Zap,
  MessageSquare, ThumbsUp, ExternalLink, Brain, Quote, Target, BookOpen
} from "lucide-react";
          ...
          <>
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <Link to="/question-bank" className="text-gray-400 hover:text-text-primary transition-colors mt-1">
                  <ArrowLeft size={18} />
                </Link>
                <h1 className="text-xl font-bold text-text-primary leading-tight">{problem.question_title || problem.question}</h1>
              </div>
            ...
            {dsaGuide.approach || dsaGuide.data_structures?.length > 0 || dsaGuide.patterns?.length > 0 || dsaGuide.tips?.length > 0 && (
              <div className="bg-space-void/60 border border-space-border rounded-xl p-4 space-y-4">
                {/* Motivational Quote */}
                <div className="flex items-start gap-3 bg-cyber-blue/5 border border-cyber-blue/20 rounded-xl p-4">
                  <Quote size={20} className="text-cyber-blue mt-0.5 shrink-0" />
                  <div>
                    <p className="text-sm text-gray-200 italic">
                      "Every expert was once a beginner. Break the problem down, trust the process, and code with confidence."
                    </p>
                    <span className="text-[10px] text-gray-500 mt-1 block">Daily Motivation</span>
                  </div>
                </div>
                ...
                {dsaGuide.approach && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-300 mb-2 flex items-center gap-2">
                      <Target size={12} className="text-cyber-blue" /> Approach
                    </h4>
                    <p className="text-xs text-gray-400 leading-relaxed">{dsaGuide.approach}</p>
                  </div>
                )}
                ...
                {dsaGuide.data_structures?.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-300 mb-2 flex items-center gap-2">
                      <BookOpen size={12} className="text-cyber-blue" /> Data Structures
                    </h4>
                    <div className="flex flex-wrap gap-1.5">
                      {dsaGuide.data_structures.map((ds, i) => (
                        <span key={i} className="text-[10px] px-2 py-1 bg-cyber-blue/10 border border-cyber-blue/30 text-cyber-blue rounded-full">
                          {ds}
                        </span>
                      ))}
                    </div>
                ...
                {dsaGuide.patterns?.length > 0 && (
                  <div>
                    <h4 className="text-xs font-semibold text-gray-300 mb-2 flex items-center gap-2">
                      <Brain size={12} className="text-cyber-blue" /> Patterns
                    </h4>
                    <div className="flex flex-wrap gap-1.5">
                      {dsaGuide.patterns.map((pattern, i) => (
                        <span key={i} className="text-[10px] px-2 py-1 bg-purple-900/20 border border-purple-800/30 text-purple-400 rounded-full">
                          {pattern}
                        </span>
                      ))}
                    </div>
                  ...
                  {similarProblems.map((sp, i) => (
                    <Link key={i} to={`/solve/${sp.id}`} className="flex items-center justify-between py-1.5 px-2 rounded hover:bg-gray-800/50 transition-colors group">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-gray-300 group-hover:text-text-primary">{sp.question_title || sp.question}</span>
                      </div>
