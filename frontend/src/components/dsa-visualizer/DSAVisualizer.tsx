...
const sortingAlgorithms: Record<string, AlgorithmMeta> = {
  bubbleSort: {
    name: "Bubble Sort",
...
const graphTraversalAlgorithms: Record<string, Omit<AlgorithmMeta, "stable">> = {
  dfs: {
...
const arrayData: Record<string, number[]> = {
  reverse: [9, 8, 7, 6, 5, 4, 3, 2, 1],
  random: [5, 2, 8, 1, 9, 3, 7, 4, 6],
  sorted: [1, 2, 3, 4, 5, 6, 7, 8, 9],
};

const sortingSteps: Record<string, StepData[]> = {
  bubbleSortSteps: [
...
export default function DSAVisualizer() {
  ...
  const handleSortAlgorithm = (algo: string) => {
  ...
  const handleGraphAlgorithm = (algo: string) => {
    setSelectedAlgorithm(algo);
    setShowGraphs(true);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="border-b bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-6">
          ...
          <p className="text-gray-600 mt-1">
            Interactive algorithm visualizations for BountyCode SDE preparation
          </p>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto p-4">
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(Object.keys(sortingAlgorithms) as (keyof typeof sortingAlgorithms)[]).map((key) => (
              <div
                key={key}
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      {step.swaps && step.swaps.length > 0 && (
                        <div>
                          <p className="text-sm text-gray-500 mb-2">Swaps:</p>
                          <div className="flex flex-wrap gap-2">
                      ...
                      {step.moved && step.moved.length > 0 && (
                        <div>
                          <p className="text-sm text-gray-500 mb-2">Moved:</p>
                          <div className="flex flex-wrap gap-2">
                            {step.moved.map((m, i) => (
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {(Object.keys(graphTraversalAlgorithms) as (keyof typeof graphTraversalAlgorithms)[]).map((key) => (
              <div
                key={key}
      ...
      <footer className="border-t pt-6 text-center text-sm text-gray-500">
        <p>DSA Visualizer — BountyCode SDE Preparation</p>
      </footer>
    </div>
  );
}
