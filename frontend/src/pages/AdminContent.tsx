...
const CATEGORY_COLORS = {
  dsa: "bg-indigo-500/10 text-indigo-400 border-indigo-500/30",
  aptitude: "bg-sky-500/10 text-sky-400 border-sky-500/30",
  "soft-skills": "bg-amber-500/10 text-amber-400 border-amber-500/30",
  "system-design": "bg-purple-500/10 text-purple-400 border-purple-500/30",
...
function Field({ label, children }) {
  return (
    <label className="block mb-4">
      <span className="block text-xs font-mono uppercase tracking-[0.2em] text-gray-500 mb-1.5">{label}</span>
      {children}
    </label>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className={`relative w-full ${wide ? "max-w-3xl" : "max-w-xl"} max-h-[90vh] overflow-y-auto rounded-2xl bg-slate-900 border border-white/10 shadow-2xl`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 sticky top-0 bg-slate-900 rounded-t-2xl z-10">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-white/5 transition-all" aria-label="Close">
            <X size={18} />
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 px-4 py-8 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        ...
        <div className="mt-6 flex gap-2 bg-slate-900/60 border border-white/10 rounded-xl p-1.5 w-fit">
      ...
      {error && (
        <div className="mb-6 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 text-sm font-mono flex items-center justify-between">
          <span>{error}</span>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-indigo-500/90 hover:bg-indigo-500 text-white text-sm font-semibold transition-all">
              <Plus size={16} />
              New Module
            </button>
          </div>
                ...
                <motion.div
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 flex-wrap">
                      ...
                      <span className="text-[10px] font-mono text-gray-500">order #{c.order}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-300 text-xs font-mono hover:text-white hover:border-indigo-500/40 transition-all">
                      <Pencil size={13} />
                      Edit
                    </button>
                    <button
                      onClick={() => handleContentDelete(c)}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-gray-300 text-xs font-mono hover:text-red-400 hover:border-red-500/40 transition-all">
                      <Trash2 size={13} />
                      Delete
                    </button>
                  </div>
      ...
      {tab === "assignments" && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          <div className="flex items-center justify-between mb-5">
            <p className="text-sm font-mono text-gray-500">{assignments.length} assignments</p>
            <button
                  ...
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        {a.content_title && (
                          <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono border bg-indigo-500/10 text-indigo-400 border-indigo-500/30">
                            {a.content_title}
                          </span>
                        )}
                      </div>
                      <p className="mt-2 text-sm text-gray-400">{a.description || "No description"}</p>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-xs font-mono hover:text-white hover:border-emerald-500/40 transition-all">
        ...
        {activeSubmissions && (
          <Modal
            ...
            {submissionsLoading ? (
              ...
              <div className="text-center py-14 text-gray-500 font-mono text-sm">No submissions yet.</div>
            ) : (
              <div className="space-y-4">
                {submissions.map(s => (
                  <div key={s.id} className="rounded-xl bg-slate-950/50 border border-white/10 p-4">
                    <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="w-7 h-7 rounded-full bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center text-[10px] font-mono text-indigo-400">
                          {(s.user_name || "?").charAt(0).toUpperCase()}
                        </span>
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-text-primary truncate">{s.user_name || "Unknown"}</p>
                          <p className="text-[10px] font-mono text-gray-500">{s.user_email || s.user_id}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <StatusBadge status={s.status} />
                        <span className="text-[10px] font-mono text-gray-500">submitted {formatDate(s.submitted_at)}</span>
                      </div>
                    </div>

                    <div className="rounded-lg bg-white/[0.03] border border-white/5 p-3 text-sm text-gray-300 whitespace-pre-wrap mb-3">
                    ...
                    {s.status === "graded" ? (
                      <div className="flex items-start gap-4 flex-wrap">
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
                          ...
                          <span className="text-[10px] font-mono text-gray-500">/ {activeSubmissions.max_score}</span>
                        </div>
                        <div className="flex-1 min-w-[200px]">
                          <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-gray-500 mb-1">Feedback</p>
                          <p className="text-sm text-gray-300 whitespace-pre-wrap">{s.feedback || "—"}</p>
                        </div>
                      </div>
                    ) : (
                      <div className="grid md:grid-cols-[120px_1fr_auto] gap-3 items-start">
                        <div>
                          <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-gray-500 mb-1">Score</p>
                          <input
                            type="number"
                            min="0"
                        ...
                        <button
                          ...
                          className="mt-6 flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/90 hover:bg-emerald-500 text-white text-sm font-semibold transition-all">
  ...
  return (
    <form
      ...
      <div className="flex justify-end gap-3 pt-2">
        <button type="button" onClick={onCancel} className="px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-300 text-sm font-mono hover:text-white transition-all">
          Cancel
        </button>
        <button type="submit" className="px-4 py-2.5 rounded-xl bg-indigo-500/90 hover:bg-indigo-500 text-white text-sm font-semibold transition-all">
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex justify-end gap-3 pt-2">
        ...
        <button type="submit" disabled={saving} className="px-4 py-2.5 rounded-xl bg-emerald-500/90 hover:bg-emerald-500 text-white text-sm font-semibold transition-all disabled:opacity-50">
