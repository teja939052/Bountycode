...
export default function Predictor() {
  ...
  const handlePracticeForRole = async () => {
    ...
    try {
      const data = await api.createPracticeSession(selectedCompany, role);
              ...
              {prediction.skill_scores && (
                <AnimatedCard className="card">
                  ...
                  <div className="space-y-3">
                    {Object.entries(prediction.skill_scores).map(([skill, score], i) => (
                      <div key={skill}>
                        ...
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                          <motion.div
                            className={`h-2 rounded-full ${score >= 8 ? "bg-green-500" : score >= 5 ? "bg-yellow-500" : "bg-red-500"}`}
                            initial={reduced ? { width: `${score * 10}%` } : { width: 0 }}
