...
export default function IndianPlacement() {
  ...
  const loadCompanies = async () => {
    ...
    try {
      ...
      if (data?.companies) {
        ...
        const res = await fetch('/api/indian-placement/companies');
  ...
  const selectCompany = async (companyId) => {
    ...
    try {
      const [detailRes, hrRes, codingRes] = await Promise.all([
        fetch(`/api/indian-placement/${companyId}`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`/api/indian-placement/${companyId}/hr-questions`).then(r => r.ok ? r.json() : null).catch(() => null),
        fetch(`/api/indian-placement/${companyId}/coding-patterns`).then(r => r.ok ? r.json() : null).catch(() => null),
      ]);
      setDetail(detailRes);
      setHrData(hrRes);
  ...
  const startMock = async () => {
    setMockLoading(true);
    try {
      const res = await fetch('/api/indian-placement/start-mock', {
        method: 'POST',
        ...
        {!selected ? (
          ...
          <div>
            ...
            <div className="flex items-center gap-4 mb-5 flex-wrap">
              ...
              <div>
                <h2 className="text-xl font-display font-bold text-white">{detail?.name || company?.name}</h2>
