      ...
      <div className="relative mx-auto max-w-2xl px-4 py-10">
        <div className="text-center mb-8">
          <motion.div
            ...
            className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-[0.3em] text-indigo-300/80 mb-2"
          >
            <Sparkles size={14} />
            The Journey
            <Sparkles size={14} />
          </motion.div>
          <h1 className="text-4xl font-bold font-display bg-gradient-to-r from-emerald-400 to-indigo-400 bg-clip-text text-transparent">
        ...
        {nextRegion && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 mb-10"
          >
            ...
            <div className="h-3 rounded-full bg-slate-800 overflow-hidden border border-white/10">
              <motion.div
                className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-indigo-500 shadow-[0_0_12px_rgba(52,211,153,0.5)]"
                initial={{ width: 0 }}
            ...
            return (
              <div key={region.id} className="flex flex-col items-center w-full">
                <div id={`journey-node-${region.id}`} className="w-full max-w-xl scroll-mt-24">
                  ...
                  >
                    {isCurrent && (
                      <div className="pulse-ring absolute inset-0 rounded-2xl text-indigo-400" />
                    )}
                    ...
                    <div className="relative flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <h3 className={`font-semibold text-lg ${revealed ? "text-slate-100" : "text-slate-500"}`}>
                            {region.name}
                          </h3>
                        ...
                        {isLocked ? (
                          ...
                          <div className="flex items-center gap-1.5 mt-1 text-xs text-emerald-400/80 font-mono">
                            <MapPin size={12} />
                      ...
                      <div className="flex flex-col items-end gap-2">
                        {regionDone ? (
                          <span className="flex items-center gap-1 text-xs font-semibold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-3 py-1.5">
                            <Check size={13} />
                            Done
                          </span>
                        ) : isLocked ? (
                          <span className="flex items-center gap-1 text-xs font-semibold text-slate-500 bg-slate-800 border border-white/10 rounded-xl px-3 py-1.5">
                            <Lock size={13} />
                            Locked
                          </span>
                        ) : isCurrent ? (
                          <span className="flex items-center gap-1 text-xs font-semibold text-indigo-300 bg-indigo-500/10 border border-indigo-500/30 rounded-xl px-3 py-1.5">
                            <MapPin size={13} />
                            Here
                          </span>
                        ) : (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold transition-all bg-gradient-to-r from-emerald-500 to-indigo-500 text-white hover:opacity-90 shadow-[0_0_16px_rgba(16,185,129,0.35)] disabled:opacity-60"
                          >
                            {moving === region.id ? (
                    ...
                    {isCurrent && (
                      ...
                      >
                        <span className="drop-shadow-[0_0_14px_rgba(52,211,153,0.7)]">🧙</span>
                      </motion.div>
                    )}
                  </motion.div>
                </div>
        ...
        {current && (
          <>
            <motion.section
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 mt-10"
            >
              <h4 className="flex items-center gap-2 text-sm font-semibold text-slate-200 mb-4">
                ...
                {current.buildings.map((b) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-xs font-semibold text-emerald-300">{meta?.label}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <motion.section
              ...
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              className="rounded-2xl border border-white/10 bg-slate-900/60 p-5 mt-4 mb-6"
            >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        {done ? (
                          <span className="w-6 h-6 flex items-center justify-center rounded-full bg-emerald-500/20 text-emerald-400">
                            <Check size={14} />
                          </span>
                        ) : (
                          <span className="w-6 h-6 flex items-center justify-center rounded-full bg-indigo-500/20 text-indigo-300">
                            <Sparkles size={13} />
                          </span>
                        )}
                        <span className={`text-sm ${done ? "text-emerald-300 line-through" : "text-slate-300"}`}>
                          {quest}
                        </span>
                      </div>
                      {done ? (
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-emerald-500 to-indigo-500 text-white hover:opacity-90 disabled:opacity-60 whitespace-nowrap"
                        >
                          {busyQuest === idx ? (
                            <span className="spinner-border spinner-border-sm" />
                          ) : (
      ...
      <AnimatePresence>
        {flash && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <div className="absolute inset-0 bg-gradient-to-b from-emerald-500/10 via-transparent to-indigo-500/10" />
            <motion.div
              initial={{ scale: 0.2, opacity: 0, rotate: -12 }}
              animate={{ scale: 1, opacity: 1, rotate: 0 }}
              exit={{ scale: 1.8, opacity: 0 }}
              transition={{ type: "spring", stiffness: 260, damping: 18 }}
              className="relative text-center rounded-3xl border border-white/10 bg-slate-900/90 px-10 py-8 shadow-[0_0_60px_rgba(52,211,153,0.35)]"
            >
              <div className="text-6xl mb-3">✨</div>
              <div className="text-white font-bold text-lg">{flash.quest}</div>
              {flash.xp > 0 && (
                <div className="mt-1 text-emerald-400 font-mono font-semibold">+{flash.xp} XP</div>
              )}
              {flash.fog && (
                <div className="mt-2 flex items-center justify-center gap-2 text-sm text-indigo-300 font-mono">
                  <Lock size={13} />
