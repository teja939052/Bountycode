...
export default function CodingChallenge() {
  ...
  if (step === "select") {
    return (
      <div className="min-h-screen py-12 px-4">
        <CelebrationOverlay show={showCelebration} type="perfect" message="Hyperdrive Engaged!" />
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              <button
                ...
                className={`p-3 rounded-lg text-left transition-all border-2 ${
                  !company ? "border-cyber-green bg-cyber-green/10" : "border-space-border hover:border-gray-600"
                }`}
              >
                <p className="font-display font-bold text-white text-sm">General</p>
              ...
              {COMPANIES.map((c) => (
                <button
                  ...
                  className={`p-3 rounded-lg text-left transition-all border-2 ${
                    company === c.id ? "border-cyber-green bg-cyber-green/10" : "border-space-border hover:border-gray-600"
                  }`}
                >
          ...
          <div className="card mb-6">
            <h3 className="font-display font-bold text-white text-sm mb-4">Select Topic</h3>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-3">
              {topics.map((topic) => (
                <button
                  ...
                  className={`p-3 rounded-lg text-left transition-all border-2 ${
                    selectedTopic === topic.id ? "border-cyber-green bg-cyber-green/10" : "border-space-border hover:border-gray-600"
                  }`}
                >
                  <p className="font-mono text-xs text-white">{topic.name}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6 mb-6">
            <div className="card">
              <h3 className="font-display font-bold text-white text-sm mb-4">Difficulty</h3>
            ...
            <div className="card">
              <h3 className="font-display font-bold text-white text-sm mb-4">Language</h3>
              <div className="flex gap-3">
                {["python", "javascript", "java", "cpp"].map((lang) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg font-mono text-xs capitalize transition-colors ${
                      language === lang ? "bg-cyber-green text-space-void" : "bg-space-panel border border-space-border text-gray-500"
                    }`}
                  >
                    {lang}
                  </button>
  ...
  if (step === "coding" && challenge) {
    return (
      <div className="min-h-screen py-12 px-4">
        <CelebrationOverlay show={showCelebration} type="perfect" message="All Tests Passed!" />
        <XPPopup show={showXP} {...xpData} onClose={() => setShowXP(false)} />
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <h2 className="font-display font-bold text-white text-lg">{challenge.title}</h2>
              {company && (
            ...
            <div className="flex items-center gap-4">
              <HyperdriveStatus state={hyperdriveState} time={formatTime(timer)} />
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            {/* Problem Description */}
            <div className="card">
              <h3 className="font-display font-bold text-white text-sm mb-3">Mission Brief</h3>
              <p className="text-gray-400 font-mono text-xs whitespace-pre-wrap mb-4">{challenge.description}</p>

              {challenge.examples?.length > 0 && (
                <div className="mb-4">
                  <p className="font-display font-bold text-white text-xs mb-2">Test Cases:</p>
                  {challenge.examples.map((ex, i) => (
                    <div key={i} className="bg-space-void border border-space-border rounded-lg p-3 mb-2 font-mono text-xs">
              ...
              {challenge.constraints?.length > 0 && (
                <div className="mb-4">
                  <p className="font-display font-bold text-white text-xs mb-2">Constraints:</p>
              ...
              {/* Progressive Hints */}
              <div className="border-t border-space-border pt-4 mt-4">
            ...
            <div className="card">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-display font-bold text-white text-sm">Solution Module</h3>
                <span className="text-[10px] font-mono text-gray-500 uppercase">{language}</span>
              </div>
              <textarea
                className="w-full h-96 font-mono text-xs p-4 border border-space-border rounded-lg resize-none focus:ring-2 focus:ring-cyber-green/50 focus:border-cyber-green/50 bg-space-void text-gray-300"
      ...
      <div className="min-h-screen py-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          {testResults?.results?.length > 0 && (
            <div className="card mb-6">
              <h3 className="font-display font-bold text-white text-sm mb-3">Test Case Diagnostics</h3>
                ...
                {testResults.results.map((tc) => (
                  <div key={tc.test_case_index} className={`rounded-lg p-3 text-xs font-mono ${tc.passed ? "bg-cyber-green/5 border border-cyber-green/20" : "bg-cyber-red/5 border border-cyber-red/20"}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-display font-bold text-white">TC-{tc.test_case_index}</span>
          ...
          {interviewerEval && (
            <div className="card mb-6">
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className="text-sm font-display font-bold text-white">Score: {interviewerEval.score}/10</span>
              </div>
              <div className="grid sm:grid-cols-2 gap-3 mb-4">
                <div className="bg-space-void border border-space-border rounded-lg p-3">
                  <p className="text-[10px] font-mono text-gray-500 mb-1">Time Complexity</p>
                  <p className="font-mono text-xs text-white">{interviewerEval.time_complexity}</p>
                </div>
                <div className="bg-space-void border border-space-border rounded-lg p-3">
                  <p className="text-[10px] font-mono text-gray-500 mb-1">Space Complexity</p>
                  <p className="font-mono text-xs text-white">{interviewerEval.space_complexity}</p>
                </div>
              </div>
