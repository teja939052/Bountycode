...
function Empty({ children }) {
  return <div className="text-slate-500 text-sm py-4 text-center">{children}</div>;
}
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="text-slate-300 mb-2">{error || "Profile not found"}</div>
          <Link to="/profile/steam" className="text-emerald-400 hover:underline">
            Back to my profile
          </Link>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-5">
        {queryUser && (
          <div className="flex items-center justify-between">
            <span className="text-xs text-slate-400">Viewing public profile</span>
            <Link to="/profile/steam" className="text-sm text-emerald-400 hover:underline">
              Back to my profile
            </Link>
          </div>
        )}
        ...
        <Card className="relative overflow-hidden">
          <div className="absolute inset-0 bg-gradient-to-br from-emerald-900/40 via-transparent to-slate-900 pointer-events-none" />
          <div className="relative flex items-center gap-4">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-teal-700 flex items-center justify-center text-4xl font-black text-slate-950 shadow-lg">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-sm text-slate-400">
                ...
                {gamification.titles && gamification.titles.length > 0 && (
                  <span className="text-emerald-400">{gamification.titles[0]}</span>
                )}
              </div>
              {user.created_at && (
                <div className="text-xs text-slate-500 mt-1">Joined {user.created_at}</div>
              )}
            </div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <div className="flex items-center justify-between text-sm mb-2">
            <span className="text-slate-300 font-semibold">Level {gamification.level}</span>
            <span className="text-emerald-400 font-semibold">{gamification.xp.toLocaleString()} XP</span>
          </div>
        ...
        <Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-2xl font-bold text-emerald-400">{gamification.streak}</span>
            <span className="text-sm text-slate-400">day{gamification.streak === 1 ? "" : "s"} in a row</span>
          </div>
        </Card>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
              {badges.slice(0, 15).map((b, i) => (
                <div
                  key={i}
                  title={badgeLabel(b)}
                  className="flex flex-col items-center gap-1 bg-slate-800/60 rounded-xl py-3 px-2"
                >
                  ...
                  <div className="text-[10px] text-slate-300 text-center leading-tight line-clamp-2">
                    {badgeLabel(b)}
                  </div>
                </div>
              ))}
          ...
          {!guild ? (
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-emerald-400">{guild.level}</div>
                <div className="text-xs text-slate-400 uppercase">Lv</div>
              </div>
            </div>
          )}
        </Card>
            ...
            <div className="space-y-3">
              {showcase.map((p) => (
                <Link
                  key={p.id}
                  to={`/showcase/${p.id}`}
                  className="flex items-center justify-between bg-slate-800/60 rounded-xl px-4 py-3 hover:bg-slate-800 transition-colors"
                >
                  <div>
                    <div className="font-semibold">{p.title}</div>
                    <div className="text-xs text-slate-400 mt-0.5">{p.language}</div>
                  </div>
                ...
                {recentActivity.map((d, i) => (
                  <div key={d.date} className="flex-1 flex flex-col items-center gap-1">
                    <div className="w-full flex items-end" style={{ height: "72px" }}>
                      <div
                        className="w-full bg-gradient-to-t from-emerald-600 to-teal-400 rounded-t-md"
                        style={{
