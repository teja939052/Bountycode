...
const DIFFICULTY_BADGES = {
  "Beginner": "bg-emerald-100 text-nature-blossom border-emerald-500/30",
  "Beginner to Advanced": "bg-gradient-to-r from-emerald-100 to-blue-100 text-transparent bg-clip-text border-blue-500/30",
  "Beginner to Intermediate": "bg-emerald-100 text-nature-blossom border-emerald-500/30",
  "Intermediate": "bg-amber-100 text-nature-sun border-amber-500/30",
  "Intermediate to Advanced": "bg-amber-100 text-nature-sun border-amber-500/30",
  "Advanced": "bg-red-100 text-red-600 border-red-500/30",
  "Beginner to Expert": "bg-gradient-to-r from-emerald-100 via-amber-100 to-red-100 text-transparent bg-clip-text border-nature-leaf/30",
};
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface-base">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-sm font-mono text-text-muted">Loading journeys...</span>
        </div>
      </div>
    );
  }
...
<div className="p-3 bg-gradient-to-br from-[#D9EFCF] to-[#EDF5E6] rounded-2xl border border-nature-leaf/30">
          <Route className="w-7 h-7 text-nature-blossom" />
            </div>
            <div>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {["All", "Beginner", "Intermediate", "Advanced"].map((filter) => (
              <button key={filter} className="px-3 py-1.5 text-xs font-medium rounded-full bg-white border border-nature-leaf/20 text-text-secondary hover:bg-surface-card transition-colors">
                {filter}
              </button>
            ))}
          </div>
          ...
          {journeys.map((journey, idx) => {
            ...
            return (
              ...
              >
                ...
                <div className="relative bg-white border border-nature-leaf/20 rounded-2xl p-5 hover:border-nature-leaf/30 transition-all duration-300 hover:shadow-xl hover:shadow-black/5 hover:-translate-y-0.5">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`px-2.5 py-1 rounded-full text-[10px] font-semibold border ${DIFFICULTY_BADGES[journey.difficulty] || "bg-surface-card text-text-muted border-nature-leaf/20"}`}>
                      {journey.difficulty}
                    </span>
                  </div>

                  <h3 className="text-lg font-bold text-text-primary mb-1.5 group-hover:text-nature-blossom transition-colors">
                    {journey.title}
                  </h3>
                  ...
                  <div className="flex items-center gap-4 text-xs text-text-muted mb-4">
                    <span className="flex items-center gap-1.5">
                      <BookOpen className="w-3.5 h-3.5 text-nature-blossom/60" />
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono text-nature-blossom font-bold">{journey.progress_pct}%</span>
                    </div>
                    <div className="h-1.5 bg-[#EDEAE0] rounded-full overflow-hidden">
                      <motion.div
                  ...
                  <div className="mt-4 pt-4 border-t border-[#EDEAE0] flex items-center justify-between">
                    <span className="text-[11px] text-text-muted font-mono">
                      {journey.lessons_completed}/{journey.total_lessons} done
                    </span>
                    <div className="flex items-center gap-1 text-[11px] text-nature-blossom font-medium opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            >
              {detailLoading ? (
                <div className="flex items-center justify-center py-20">
                  <div className="w-8 h-8 border-4 border-nature-leaf/30 border-t-[#4F8F57] rounded-full animate-spin" />
                </div>
              ) : journeyDetail ? (
                <div className="p-6 md:p-8">
                  <div className="flex items-start gap-4 mb-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className="text-2xl font-bold text-text-primary mb-1">{journeyDetail.title}</h2>
                      <p className="text-text-muted text-sm">{journeyDetail.description}</p>
                      <div className="flex flex-wrap items-center gap-3 mt-3">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${DIFFICULTY_BADGES[journeyDetail.difficulty] || "bg-surface-card text-text-muted border-nature-leaf/20"}`}>
                  ...
                  <div className="bg-[#F7F5EE] rounded-xl p-5 mb-6 border border-nature-leaf/20">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-semibold text-text-primary">Overall Progress</span>
                      <span className="text-lg font-bold font-mono bg-gradient-to-r from-[#2E6B35] to-[#7BB661] bg-clip-text text-transparent">
                  ...
                  {journeyDetail.languages && journeyDetail.languages.length > 0 && (
                    <div>
                      <h3 className="text-sm font-semibold text-text-secondary mb-4 flex items-center gap-2">
                        <BookOpen className="w-4 h-4 text-nature-blossom" /> Languages in this journey
                      </h3>
                      <div className="space-y-3">
                        {journeyDetail.languages.map((lang) => (
                          <div key={lang.id} className="bg-[#F7F5EE] rounded-xl p-4 border border-nature-leaf/20 hover:border-nature-leaf/30 transition-colors">
                            <div className="flex items-center gap-3 mb-3">
                              <span className="text-xl">{lang.icon}</span>
                              <div className="flex-1 min-w-0">
                                <h4 className="font-semibold text-text-primary text-sm">{lang.name}</h4>
                                <p className="text-xs text-text-muted font-mono">
                                  {lang.completed_lessons}/{lang.total_lessons} lessons
                                </p>
                              </div>
                              <span className="text-xs font-mono font-bold text-nature-blossom">{lang.progress_pct}%</span>
                            </div>
                ...
                <div className="text-center py-12 text-text-muted">Journey not found.</div>
              )}
            </motion.div>
          </motion.div>
        )}
