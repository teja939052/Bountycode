  ...
  if (!project) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4">
        ...
        <h3 className="text-lg font-semibold text-brand-secondary mb-2">{error || "Project not found"}</h3>
        <button
          onClick={() => navigate("/showcase")}
          className="mt-4 flex items-center gap-2 px-4 py-2 rounded-xl border border-brand-primary/10 bg-surface-card/30 text-sm text-brand-secondary hover:border-emerald-500/40"
        >
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        <button
          onClick={() => navigate("/showcase")}
          className="flex items-center gap-2 text-sm text-brand-muted hover:text-brand-primary transition-colors mb-6"
        >
          <ArrowLeft className="w-4 h-4" /> Back to gallery
        </button>
        ...
        >
          <div className="flex items-start justify-between gap-4 mb-3">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={handleShare}
                className="p-2.5 rounded-xl border border-brand-primary/10 bg-surface-card/30 text-brand-muted hover:text-brand-primary hover:border-emerald-500/40 transition-all"
                title="Copy link"
              >
                {copied ? <Check className="w-4 h-4 text-brand-primary" /> : <Share2 className="w-4 h-4" />}
              </button>
              {isAuthor && (
                <button
          ...
          <div className="flex flex-wrap items-center gap-4 text-xs text-brand-muted mb-4">
          ...
          <p className="text-sm text-brand-secondary leading-relaxed mb-4">{project.description}</p>

          <div className="flex flex-wrap items-center gap-2 mb-5">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-medium border border-brand-primary/20 bg-brand-primary/10 text-brand-primary">
              {project.language}
            </span>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all ${
                liked
                  ...
                  : "border-brand-primary/10 bg-surface-card/30 text-brand-secondary hover:border-emerald-500/40 hover:text-brand-primary"
              }`}
            >
        ...
        <motion.div
          ...
          className="rounded-2xl border border-brand-primary/10 bg-surface-card/30 overflow-hidden mb-6"
        >
          <div className="flex items-center justify-between px-6 py-3 border-b border-brand-primary/10 bg-white/[0.03]">
        ...
        >
          ...
          <div className="rounded-xl border border-brand-primary/10 bg-white/[0.03] p-4 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex-1">
                <label className="block text-xs font-mono uppercase tracking-wider text-brand-muted mb-2">Your review</label>
                <textarea
                  value={comment}
                  onChange={e => setComment(e.target.value)}
                  placeholder="Share constructive feedback on this code..."
                  rows={3}
                  className="w-full rounded-xl border border-brand-primary/10 bg-surface-card/30 px-4 py-2.5 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/20 transition-all resize-none"
                />
              </div>
              <div className="flex flex-col items-start sm:items-end gap-3 shrink-0">
                <div>
                  <label className="block text-xs font-mono uppercase tracking-wider text-brand-muted mb-1.5">Rating</label>
            ...
            <div className="space-y-4">
              {project.reviews.map(review => (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-xl border border-brand-primary/10 bg-white/[0.03] p-4"
                >
                  <div className="flex items-center justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs text-brand-muted">{formatDate(review.created_at)}</span>
                    </div>
                  </div>
