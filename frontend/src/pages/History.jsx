    ...
    const loadHistory = async () => {
      ...
      try {
        const [interviews, resumes, aptitude, coding, systemDesign] = await Promise.all([
          ...
          api.get("/api/coding/history").catch(() => ({ challenges: [] })),
          api.get("/api/system-design/history").catch(() => ({ sessions: [] })),
        ]);
        setData({
    ...
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          >
            <h1 className="text-3xl font-display font-black text-white">Flight Log</h1>
                ...
                <StaggerItem key={item._id || item.interview_id}>
                  <div className="card flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-display font-bold text-white">{item.job_role}</p>
                    ...
                    <div className="flex items-center gap-4">
                      <div className="text-right">
                        <p className="text-white font-display font-bold text-lg">{item.overall_score || "—"}/10</p>
                ...
                <StaggerItem key={item._id || item.resume_id}>
                  <div className="card flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-display font-bold text-white">{item.target_role || "Resume"}</p>
                        <p className="text-gray-500 font-mono text-xs flex items-center gap-1">
                          <Calendar size={12} /> {formatDate(item.created_at)}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-4">
                      {item.ats_score && (
                        <div className="text-right">
                          <p className="font-bold text-lg text-white">{item.ats_score}%</p>
                ...
                <StaggerItem key={item._id || item.test_id}>
                  <div className="card flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-display font-bold capitalize text-white">{item.category} — {item.difficulty}</p>
                        <p className="text-gray-500 font-mono text-xs flex items-center gap-1">
                          <Calendar size={12} /> {formatDate(item.created_at)}
                        </p>
                      </div>
                    ...
                    <div className="text-right">
                      <p className="text-white font-display font-bold text-lg">{item.percentage || item.score || "—"}%</p>
                ...
                <StaggerItem key={item._id || item.challenge_id}>
                  <div className="card flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-display font-bold text-white">{item.title || "Coding Challenge"}</p>
                        <p className="text-gray-500 font-mono text-xs flex items-center gap-1">
                          <Calendar size={12} /> {formatDate(item.created_at)}
                        </p>
                      </div>
                ...
                <StaggerItem key={item._id || item.session_id}>
                  <div className="card flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-display font-bold text-white">{item.topic || "System Design"}</p>
                        <p className="text-gray-500 font-mono text-xs flex items-center gap-1">
                          <Calendar size={12} /> {formatDate(item.created_at)}
                        </p>
                      </div>
