    ...
    <div className="min-h-screen py-6 px-4 max-w-6xl mx-auto">
      ...
      <motion.div initial={reduced ? {} : { opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl font-bold text-white dark:text-white">Problem of the Day</h1>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="flex items-center gap-1 bg-surface-card/50 bg-surface-card p-1 rounded-xl">
            {["problem","code","leaderboard"].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={"flex-1 py-2 px-4 rounded-lg text-xs font-medium transition-colors " + (activeTab === tab ? "bg-white bg-surface-card/50 text-white dark:text-white shadow-sm" : "text-brand-muted dark:text-gray-400")}>
          ...
          {activeTab === "problem" && problem && (
            <AnimatedCard className="card">
              <div className="flex items-center gap-3 mb-4">
                <h2 className="text-xl font-bold text-white dark:text-white flex-1">{problem.question_title}</h2>
              ...
              {problem.examples?.length > 0 && (<div className="mt-4 space-y-3"><h3 className="font-semibold text-white dark:text-white text-sm">Examples</h3>{problem.examples.map((ex,i) => (<div key={i} className="bg-surface-base bg-surface-card/50 rounded-lg p-3 border border-brand-primary/10 dark:border-gray-700"><div className="text-xs font-mono text-brand-muted mb-1">Input: {ex.input || "N/A"}</div><div className="text-sm font-mono text-green-700 dark:text-green-400 mb-1">Output: {ex.output || "N/A"}</div>{ex.explanation && <div className="text-xs text-brand-muted mt-2">{ex.explanation}</div>}</div>))}</div>)}
          ...
          {activeTab === "code" && (
            <AnimatedCard className="card">
              <div className="flex items-center gap-2 mb-4"><Flame size={16} className="text-primary-600" /><h2 className="text-lg font-bold text-white dark:text-white">Code Editor</h2></div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <button onClick={async () => { if(!userCode.trim()) return; setCodeRunning(true); try { const d = await api.executeCompilerCode({code:userCode,language:selectedLanguage,stdin:"",timeout:10}); setRunResult(d); } catch(e) {} setCodeRunning(false); }} className="flex items-center gap-1.5 px-3 py-1.5 bg-brand-emerald hover:bg-green-700 text-white text-sm font-medium rounded-lg">Run</button>
          ...
          {activeTab === "leaderboard" && challenge.leaderboard && (
            <AnimatedCard className="card">
              <h2 className="text-lg font-bold text-white dark:text-white mb-4">Leaderboard</h2>
                ...
                <div className="space-y-2">
                  {challenge.leaderboard.map((entry, i) => (
                    <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-surface-base bg-surface-card/50">
                      ...
                      <span className="flex-1 font-medium text-sm text-white dark:text-white">{entry.user_name}</span>
        ...
        <div className="space-y-6">
          <AnimatedCard className="card"><h3 className="font-bold text-white dark:text-white mb-3">Your Stats</h3><div className="space-y-3"><div className="flex justify-between"><span className="text-sm text-brand-muted">Streak</span><span className="font-bold text-sm text-white dark:text-white">{user?.streak || 0} days</span></div><div className="flex justify-between"><span className="text-sm text-brand-muted">XP</span><span className="font-bold text-sm text-primary-600">{user?.xp || 0}</span></div><div className="flex justify-between"><span className="text-sm text-brand-muted">Level</span><span className="font-bold text-sm text-white dark:text-white">{user?.level || 1}</span></div></div></AnimatedCard>
          {!challenge.already_completed && (<AnimatedCard className="card border-2 border-primary-200 dark:border-primary-800"><div className="text-center"><Trophy size={28} className="mx-auto mb-2 text-primary-500" /><h3 className="font-bold text-white dark:text-white mb-1">Daily Bonus</h3><p className="text-xs text-brand-muted dark:text-gray-400 mb-3">Solve today for +{challenge.streak_bonus} bonus XP</p><button onClick={() => setActiveTab("code")} className="w-full py-2 bg-primary-600 hover:bg-primary-700 text-white text-sm font-medium rounded-lg transition-colors">Start Challenge</button></div></AnimatedCard>)}
        </div>
      </div>
    </div>
  );
