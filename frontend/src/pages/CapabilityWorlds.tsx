...
export default function CapabilityWorlds() {
  ...
  if (loading && !selectedWorld) {
    return <div className="min-h-screen page-surface flex items-center justify-center"><div className="w-8 h-8 border-2 border-border border-t-primary rounded-full animate-spin" /></div>;
  }
    ...
    <div className="min-h-screen page-surface">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-purple-soft border border-purple/20 rounded-full text-purple text-sm mb-4">
            <Rocket className="w-4 h-4" /> Capability Curriculum
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-3 text-text-primary">
            Train the <span className="text-primary">capabilities</span> the job requires.
          </h1>
        ...
        {daily && daily.recommendation && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            className="mb-8 p-5 bg-blue-soft border border-blue/20 rounded-2xl">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-sm text-blue mb-1">{daily.icon} Today's Mission — {daily.label}</div>
                <div className="text-lg font-semibold text-text-primary">{daily.recommendation.title}</div>
                <div className="text-sm text-text-muted mt-1">{daily.recommendation.scenario}</div>
              </div>
              <button onClick={() => { setSelectedWorld(daily.recommendation!.world_id); }}
                className="px-5 py-2 bg-blue/10 hover:bg-blue/20 rounded-lg text-blue text-sm font-medium transition-colors flex items-center gap-2">
          ...
          {worlds.map((world, i) => (
            <motion.button key={world.id}
              ...
              className={`w-full p-5 rounded-2xl border text-left transition-all flex items-center gap-5 ${
                world.unlocked ? 'bg-white border-border hover:border-primary/30 hover:shadow-card-hover' : 'bg-surface-2 border-border opacity-50 cursor-not-allowed'
              }`}>
              <div className="text-4xl">{world.icon}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-3 mb-1">
                  <span className="font-bold text-lg text-text-primary">{world.title}</span>
                  {!world.unlocked && <Lock className="w-4 h-4 text-text-muted" />}
                </div>
                <div className="text-sm text-text-muted">{world.description}</div>
                <div className="text-xs text-text-muted mt-2">{world.competencies} competencies</div>
              </div>
  ...
  return (
    <div className="min-h-screen page-surface">
      <div className="max-w-5xl mx-auto px-4 py-8">
        <button onClick={onBack} className="mb-6 px-4 py-2 text-text-muted hover:text-primary text-sm transition-colors">&larr; Back to Worlds</button>

        <div className="mb-8">
          <h2 className="text-3xl font-bold mb-2">{competencies[0] ? '' : ''}{worldId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</h2>
          <p className="text-text-muted">Complete all competencies to master this world.</p>
        </div>
        ...
        <div className="space-y-4">
          {competencies.map((comp, i) => (
            <motion.a key={comp.id}
              ...
              initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
              className="block p-5 rounded-2xl border bg-white border-border hover:border-primary/30 hover:shadow-card-hover transition-all group">
              <div className="flex items-center gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold ${
                  comp.completed ? 'bg-primary-soft text-primary-dark' : 'bg-surface-2 text-text-muted border border-border'
                }`}>
                  {comp.completed ? <CheckCircle2 className="w-6 h-6" /> : <span>{i + 1}</span>}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-lg mb-1">{comp.title}</div>
                  <div className="text-sm text-text-muted line-clamp-2">{comp.scenario}</div>
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {comp.skills_taught.map(skill => (
                      <span key={skill} className="px-2 py-0.5 bg-surface-2 border border-border rounded text-xs text-text-muted">{skill.replace(/_/g, ' ')}</span>
                    ))}
                  </div>
                </div>
                <div className="text-right flex-shrink-0">
                  <div className="text-sm text-text-muted">{comp.step_count} steps</div>
                  {comp.completed && <div className="text-sm text-primary-dark font-medium mt-1">{comp.score}%</div>}
                </div>
