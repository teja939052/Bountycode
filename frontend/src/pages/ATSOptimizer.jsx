  ...
  return (
    <div className="min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-center gap-4 mb-10">
          {[1, 2, 3].map((s) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-display font-bold ${step >= s ? "bg-cyber-orange text-space-void" : "bg-space-panel border border-space-border text-gray-500"}`}>
                {s}
              </div>
              <span className={`text-xs font-mono ${step >= s ? "text-white" : "text-gray-500"}`}>
        ...
        {step === 1 && !loading && (
          <div className="card">
            <h2 className="font-display font-bold text-white text-lg mb-4">Step 1: Upload Resume</h2>
            <div className="border-2 border-dashed border-space-border rounded-lg p-8 text-center hover:border-cyber-orange/50 transition-colors">
        ...
        {step === 2 && !loading && (
          <div className="card">
            <h2 className="font-display font-bold text-white text-lg mb-4">Step 2: Paste Mission Parameters</h2>
            <div className="bg-space-void border border-space-border rounded-lg p-3 mb-4">
              <p className="text-xs font-mono text-gray-400">
                Resume loaded: <span className="text-white">{resumeText.substring(0, 80)}...</span>
              </p>
            </div>
            <textarea
        ...
        {step === 3 && optimization && (
          <div className="space-y-6">
            ...
            <HullScanner score={optimization.ats_score} />
              ...
              <div className="card border-cyber-purple/20">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{semanticScore.semantic?.overall_score ?? 0}%</p>
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {Object.entries(semanticScore.section_scores || {}).map(([sec, val]) => (
                  ...
                  <div className="mt-4">
                    ...
                    <div className="space-y-2">
                      {semanticScore.semantic_gaps.gaps.slice(0, 5).map((g, i) => (
                        <div key={i} className="bg-space-void border border-space-border rounded-lg p-3">
              ...
              <div className="space-y-4">
                <div className="card bg-gradient-to-r from-cyber-blue/10 to-cyber-purple/10 border border-cyber-blue/20">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {Object.entries(semanticScore.breakdown || {}).map(([key, val]) => (
                      <div key={key} className="bg-space-void border border-space-border rounded-lg p-3 text-center">
                        <p className="text-[10px] font-mono text-gray-400 uppercase tracking-wider mb-1">{key.replace(/_/g, " ")}</p>
                        <p className="text-lg font-bold text-white">{val.score}</p>
                ...
                {semanticScore.semantic_sections && Object.keys(semanticScore.semantic_sections).length > 0 && (
                  <div className="card">
                    <h3 className="font-display font-bold text-white text-sm mb-3">Section Similarity</h3>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {Object.entries(semanticScore.semantic_sections).map(([section, score]) => (
                        <div key={section} className="bg-space-void border border-space-border rounded-lg p-3 text-center">
            ...
            {optimization.changes_made?.length > 0 && (
              <div className="card">
                <h3 className="font-display font-bold text-white text-sm mb-3">🔧 Modifications Applied</h3>
            ...
            {optimization.optimized_resume && (
              <div className="card">
                <h3 className="font-display font-bold text-white text-sm mb-4">📄 Optimized Manifest</h3>
                <pre className="whitespace-pre-wrap text-xs font-mono text-gray-400 bg-space-void border border-space-border p-4 rounded-lg max-h-96 overflow-y-auto">
                  {optimization.optimized_resume}
                </pre>
              </div>
            )}
