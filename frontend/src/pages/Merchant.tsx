...
const TYPE_STYLES = {
  card: "border-indigo-500/30 bg-indigo-500/10 text-indigo-400",
  xp_potion: "border-emerald-500/30 bg-emerald-500/10 text-emerald-400",
  double_xp_token: "border-amber-500/30 bg-amber-500/10 text-amber-400",
  avatar: "border-fuchsia-500/30 bg-fuchsia-500/10 text-fuchsia-400",
...
export default function Merchant() {
    ...
    return (
      <div className="min-h-screen bg-slate-950 text-slate-200 flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          <Gift className="w-12 h-12 text-emerald-400 animate-bounce" />
          <p className="text-slate-500 text-sm">Summoning the merchant...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 relative overflow-hidden">
      <div className="pointer-events-none absolute -top-32 left-1/2 -translate-x-1/2 w-[600px] h-[400px] rounded-full bg-indigo-600/20 blur-[120px]" />
      <div className="pointer-events-none absolute bottom-0 right-0 w-[400px] h-[300px] rounded-full bg-emerald-500/10 blur-[120px]" />
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          {shop && (
            <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 self-start sm:self-auto">
              <Coins className="w-5 h-5 text-emerald-400" />
              <span className="text-emerald-300 font-bold">{shop.coins}</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
          {(shop?.items || []).map((item, idx) => (
            <motion.div
              ...
              className="relative rounded-2xl border border-white/10 bg-slate-900/70 p-5 flex flex-col"
            >
              {item.discount_pct > 0 && (
              ...
              <div className="w-16 h-16 rounded-2xl bg-slate-800/80 border border-white/10 flex items-center justify-center text-4xl mb-4">
                {item.emoji}
              </div>
              <h3 className="text-base font-semibold text-white">{item.name}</h3>
              <span className={`mt-2 self-start px-2.5 py-0.5 rounded-full text-[10px] font-mono font-medium border ${TYPE_STYLES[item.type] || "border-white/10 text-slate-400"}`}>
                {TYPE_LABELS[item.type] || item.type}
              </span>
              <div className="mt-4 flex items-center gap-2">
                <Coins className="w-4 h-4 text-amber-400" />
                <span className="text-xl font-bold text-amber-300">{item.price}</span>
                {item.discount_pct > 0 && (
                  <span className="text-xs text-slate-500 line-through">{item.original_price}</span>
                )}
              </div>
              <button
                ...
                className={`mt-4 w-full flex items-center justify-center gap-2 rounded-xl py-2.5 text-sm font-semibold transition-all ${
                  item.purchased
                    ...
                    : "bg-gradient-to-r from-emerald-500 to-indigo-500 text-white hover:opacity-90 disabled:opacity-50"
                }`}
              >
                {item.purchased ? (
                  <>
        ...
        >
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-6">
            <div className="flex items-center gap-4">
              <div
                className="w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500/20 to-emerald-500/20 border border-white/10 flex items-center justify-center text-3xl"
                style={currentPerk ? { borderColor: currentPerk.border, borderWidth: 2 } : undefined}
              >
                {currentPerk ? currentPerk.emote : <Crown className="w-8 h-8 text-indigo-400" />}
              </div>
              <div>
            ...
            <span className="self-start lg:self-auto px-3 py-1.5 rounded-xl border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 text-sm font-bold">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-4">
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 text-white px-6 py-3 text-sm font-semibold hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
            >
            ...
            {nextReq && (
              <p className="text-xs text-slate-500">
                Requires{" "}
                <span className="text-slate-300 font-semibold">{nextReq.xp_threshold.toLocaleString()} XP</span>
          ...
          >
            ...
            >
              ...
              <p className="mt-3 text-sm text-slate-400 leading-relaxed">
                ...
                <span className="text-white">{nextPerk?.title}</span> title, a permanent{" "}
                <span className="text-emerald-400">x{formatMultiplier(nextPerk?.bonus_multiplier ?? 1)} XP bonus</span>,
                and the exclusive {nextPerk?.emote} border.
              </p>
              <div className="mt-6 flex gap-3 justify-end">
                <button
                  onClick={() => setConfirmReset(false)}
                  className="rounded-xl border border-white/10 bg-slate-800 px-5 py-2.5 text-sm text-slate-300 hover:bg-slate-700 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={prestiging}
                  className="flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-500 px-5 py-2.5 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50 transition-all"
                >
                  {prestiging ? (
                    <>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 inset-x-0 mx-auto z-50 w-fit max-w-[90vw] rounded-xl border border-white/10 bg-slate-900/95 px-5 py-3 text-sm text-slate-200 shadow-xl text-center"
          >
            {toast}
          </motion.div>
        )}
